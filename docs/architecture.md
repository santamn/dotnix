...
- **flake を root で評価しない**: `sudo nixos-rebuild switch --flake ~/dotnix` のように sudo 付きで起動すると評価も root で行われ、作業ツリーが dirty なとき nix が `~/dotnix/.git/objects` に root 所有のオブジェクトを書き込む。以後ユーザ権限の `git` / `nix flake update` が `insufficient permission for adding an object to repository database` で失敗するようになるため、`nixos-rebuild switch --sudo --flake ~/dotnix` (評価はユーザ、activation だけ sudo) を使うこと。壊れた場合の復旧は `sudo chown -R "$USER:$(id -gn)" ~/dotnix`
